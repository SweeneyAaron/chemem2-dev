...
//
// Copied verbatim from ChemEM/cpp/docking_v2/LbfgsRefine.h, where it was written for
// the experimental engine (improvement R1). The baseline engine now selects it with
// --local-minimiser lbfgs. Keep the two copies in sync, or hoist to a shared dir; the
// engines are deliberately isolated (separate .so, no shared objects) so a duplicated
// header is the cheaper of the two evils today.
//
// Why this exists: the default path refines poses with derivative-free Nelder-Mead,
...
// fewer evaluations, which is what makes "refine many diverse basins" affordable --
// in the baseline engine that means a larger --n-local-search at the same wall clock.
//
// Note the tension with the FD cost model: a gradient costs 2*D evaluations, so L-BFGS
// only wins if it converges in far fewer ITERATIONS than NM burns. Measure it with
// CHEMEM_DOCK_PROFILE=1 rather than assuming.
//
