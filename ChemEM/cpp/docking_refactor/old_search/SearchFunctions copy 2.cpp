...
AntColonyOptimizer::refinePoseSplitNmFromDiscrete(
    ...
    // Tunables (edit here)
    // =========================
    const int    MAX_CYCLES            = 10;      // hard cap
    const size_t MAX_WINDOWS_TO_USE    = 24;     // cap per cycle
    const double MIN_IMPROVE_KCAL      = 5e-3;   // stop if cycle improves less than this

    // Termination tolerances for NM (TR vs window)
    const double FTOL_TR               = 2e-4;
    const double FTOL_W                = 2e-4;

    // Per-coordinate xtol (converted to overall xtol by sqrt(dim))
    const double XPER_TR               = 6e-4;
    const double XPER_W                = 6e-4;

    // Initial simplex sizes in normalized [0..1] coords
    // (we'll decay them each cycle)
    const double EPS_TR0_SMALL         = 0.020;
    const double EPS_W0_SMALL          = 0.012;

    const double EPS_TR0_MANY          = 0.015; //though this should be 0.045?
    const double EPS_W0_MANY           = 0.010; //was 0.10 also need to try this at 0.045

    // Minimum eps clamp
    const double EPS_TR_MIN            = 0.005;
    const double EPS_W_MIN             = 0.004;

    // Iteration budgets (also decayed)
    const int    IT_TR0_SMALL          = 400;
    const int    IT_W0_SMALL           = 260;

    const int    IT_TR0_MANY           = 350;
    const int    IT_W0_MANY            = 220;

    const int    IT_TR_MIN             = 120;
    const int    IT_W_MIN              = 90;

    // Decay factor per cycle (0.5–0.8 are typical)
    const double EPS_DECAY             = 0.65;
    const double IT_DECAY              = 0.80;

    // Optional: if you want a second tiny TR “re-seat” after windows each cycle
    const bool   DO_POST_TR            = true;
    const double POST_TR_IT_FRAC       = 0.40;
    const double POST_TR_EPS_FRAC      = 0.75;
    ...
    // Current best in normalized delta space
    std::vector<double> x_cur(D, 0.5);
    double bestScore = eval_full(x_cur);

    // Subspace NM: optimize only dims in `idxs`, keep rest fixed at x_cur
    auto optimize_subspace = [&](const std::vector<size_t>& idxs,
                                 double eps,
                                 int iters,
                                 double ftol,
                                 double xtol) {
        ...
        std::vector<double> x0(K, 0.5);
        for (size_t i = 0; i < K; ++i) x0[i] = x_cur[idxs[i]];

        auto obj = [&](const std::vector<double>& x_sub_in) -> double {
            std::vector<double> x = x_cur;
            for (size_t i = 0; i < K; ++i) {
                x[idxs[i]] = std::clamp(x_sub_in[i], 0.0, 1.0);
            }
            return eval_full(x);
        };

        NelderMeadOptimizer nm(static_cast<int>(K));
        auto simplex = make_simplex(x0, eps);
        auto res = nm.optimize(obj, simplex, iters, ftol, xtol);

        if (res.best_value < bestScore) {
            bestScore = res.best_value;
            for (size_t i = 0; i < K; ++i) {
                x_cur[idxs[i]] = std::clamp(res.best_point[i], 0.0, 1.0);
            }
        }
    };

    const std::vector<size_t> idxTR = {0,1,2,3,4,5};

    // Torsion windows: indices into torsion list, x index = 6 + tidx
    const auto& windows = pre.ligand_score().ligand_torsion_windows;
    const bool hasWindows = !windows.empty();
    const size_t nWinUse = std::min(MAX_WINDOWS_TO_USE, windows.size());

    // Cycle parameter generator
    auto cycle_params = [&](int cyc, double& epsTR, int& itTR, double& epsW, int& itW) {
        const double epsTR0 = manyTors ? EPS_TR0_MANY : EPS_TR0_SMALL;
        const double epsW0  = manyTors ? EPS_W0_MANY  : EPS_W0_SMALL;
        const int itTR0     = manyTors ? IT_TR0_MANY   : IT_TR0_SMALL;
        const int itW0      = manyTors ? IT_W0_MANY    : IT_W0_SMALL;

        const double powE = std::pow(EPS_DECAY, static_cast<double>(cyc));
        const double powI = std::pow(IT_DECAY,  static_cast<double>(cyc));

        epsTR = std::max(EPS_TR_MIN, epsTR0 * powE);
        epsW  = std::max(EPS_W_MIN,  epsW0  * powE);

        itTR  = std::max(IT_TR_MIN, static_cast<int>(std::lround(itTR0 * powI)));
        itW   = std::max(IT_W_MIN,  static_cast<int>(std::lround(itW0  * powI)));
    };
    ...
    while (cyc < MAX_CYCLES) {
        const double score_before = bestScore;

        double epsTR, epsW;
        int itTR, itW;
        cycle_params(cyc, epsTR, itTR, epsW, itW);
        ...
        optimize_subspace(
            ...
            itTR,
            FTOL_TR,
            xtol_from_percoord(XPER_TR, idxTR.size())
        );
        /*
        if (nTors > 0) {
            if (hasWindows) {
                // (2) Windowed torsion micro-opts
                for (size_t w = 0; w < nWinUse; ++w) {
                    const auto& win = windows[w];

                    std::vector<size_t> idxW;
                    idxW.reserve(4);
                    for (int k = 0; k < 4; ++k) {
                        const int tidx = win[k];
                        if (tidx < 0) continue;
                        if (static_cast<size_t>(tidx) >= nTors) continue;
                        idxW.push_back(6 + static_cast<size_t>(tidx));
                    }
                    std::sort(idxW.begin(), idxW.end());
                    idxW.erase(std::unique(idxW.begin(), idxW.end()), idxW.end());
                    if (idxW.empty()) continue;
                    ...
                    optimize_subspace(
                        ...
                        itW,
                        FTOL_W,
                        xtol_from_percoord(XPER_W, idxW.size())
                    );
                }
            } else {
                // Fallback: no windows -> small global torsion pass
                std::vector<size_t> idxAll;
                idxAll.reserve(nTors);
                for (size_t t = 0; t < nTors; ++t) idxAll.push_back(6 + t);

                const int iters = std::min(700, std::max(IT_W_MIN, 180 + 22 * static_cast<int>(nTors)));
                optimize_subspace(
                    idxAll,
                    epsW,
                    iters,
                    FTOL_W,
                    xtol_from_percoord(XPER_W, idxAll.size())
                );
            }
        }*/
        if (nTors > 0) {
            if (hasWindows) {
                // (2) Windowed torsion micro-opts
                for (size_t w = 0; w < nWinUse; ++w) {
                    const auto& win = windows[w]; // win: std::vector<int>
        
                    std::vector<size_t> idxW;
                    idxW.reserve(win.size());
        
                    for (int tidx : win) {
                        if (tidx < 0) continue; // keep if you still allow padding
                        if (static_cast<size_t>(tidx) >= nTors) continue;
                        idxW.push_back(6 + static_cast<size_t>(tidx));
                    }
        ...
        // Optional: reseat TR after torsion adjustments
        if (DO_POST_TR) {
            const int itTR2 = std::max(IT_TR_MIN, static_cast<int>(std::lround(itTR * POST_TR_IT_FRAC)));
            const double epsTR2 = std::max(EPS_TR_MIN, epsTR * POST_TR_EPS_FRAC);
        ...
        const double improve = score_before - bestScore;
        if (improve < MIN_IMPROVE_KCAL) {
            break; // no meaningful improvement => stop cycling
        }
        ++cyc;
    }

    // Convert refined normalized deltas back to discrete bins for pheromone update
    std::vector<double> disc_refined = convertRealSpaceToDiscrete(discSol, x_cur);

    // Apply best deltas to the returned molecule
    restore_coords(conf, baseline_coords);
    applyNormalizedDeltasOnBaseline(pre, conf, x_cur, base_tors_deg);
...
AntColonyOptimizer::refinePoseSplitNmFromDiscrete(
    ...
    const int    TR_TRIES_PER_CYCLE  = 18;
    const int    W_TRIES_PER_WINDOW  = 10;
    const int    MAX_EVALS           = 1000;

    double stepTR0 = 0.030;
    double stepW0  = 0.018;
    const double STEP_TR_MIN = 0.006;
    const double STEP_W_MIN  = 0.005;
    const double STEP_DECAY  = 0.75;

    const bool   ALLOW_UPHILL = true;
    double       TEMP0        = 0.25;
    const double TEMP_MIN     = 0.03;
    const double TEMP_DECAY   = 0.80;

    const size_t MAX_WINDOWS_TO_USE = 28;
    const bool   FALLBACK_CHUNK_WINDOWS = true;
    // =========================
    ...
    // Deterministic RNG seed based on pose
    uint64_t seed = splitmix64(m_baseseed ^ (0x9e3779b97f4a7c15ULL + static_cast<uint64_t>(D)));
    auto mix_q = [&](double v, double scale) {
        const int64_t q = static_cast<int64_t>(std::llround(v * scale));
        seed = splitmix64(seed ^ static_cast<uint64_t>(q));
    ...
    auto eval_full = [&](const std::vector<double>& x_norm_in) -> double {
        std::vector<double> x = x_norm_in;
        for (double &v : x) v = clamp01(v);

        restore_coords(conf, baseline_coords);
        applyNormalizedDeltasOnBaseline(pre, conf, x, base_tors_deg);
        ++evals;
        return scorer.score(conf, rep_max, map_score_function);
    };
    ...
    // Windows
    const auto &wins_src = pre.ligand_score().ligand_torsion_windows;
    std::vector<std::array<int,4>> wins_use;

    if (!wins_src.empty()) {
        const size_t nUse = std::min(MAX_WINDOWS_TO_USE, wins_src.size());
        wins_use.assign(wins_src.begin(), wins_src.begin() + nUse);
    } else if (FALLBACK_CHUNK_WINDOWS && nTors > 0) {
        for (size_t t0 = 0; t0 < nTors; t0 += 4) {
            std::array<int,4> w = {-1,-1,-1,-1};
            int k = 0;
            for (size_t t = t0; t < std::min(nTors, t0 + 4); ++t) w[k++] = static_cast<int>(t);
            wins_use.push_back(w);
            if (wins_use.size() >= MAX_WINDOWS_TO_USE) break;
        }
    }

    auto accept = [&](double candScore, double curScore, double temp) -> bool {
        if (candScore < curScore) return true;
        if (!ALLOW_UPHILL) return false;
        if (temp <= 1e-12) return false;
        const double d = candScore - curScore;
        const double p = std::exp(-d / temp);
        return (U01(rng) < p);
    };

    auto propose_on_dims = [&](const std::vector<size_t> &dims, double step) -> std::vector<double> {
        std::vector<double> x = x_cur;
        for (size_t d : dims) x[d] = clamp01(x[d] + step * U(rng));
        return x;
    };

    const std::vector<size_t> dimsTR = {0,1,2,3,4,5};

    double stepTR = stepTR0;
    double stepW  = stepW0;
    double temp   = TEMP0;

    for (int cyc = 0; cyc < MAX_CYCLES && evals < MAX_EVALS; ++cyc) {
        const double score_before = bestScore;

        // TR
        for (int t = 0; t < TR_TRIES_PER_CYCLE && evals < MAX_EVALS; ++t) {
            auto x_try = propose_on_dims(dimsTR, stepTR);
            const double s_try = eval_full(x_try);
            if (accept(s_try, bestScore, temp)) {
                x_cur = std::move(x_try);
                bestScore = s_try;
            }
        }

        // windows
        for (const auto &w : wins_use) {
            if (evals >= MAX_EVALS) break;

            std::vector<size_t> dimsW;
            dimsW.reserve(4);
            for (int k = 0; k < 4; ++k) {
                const int tidx = w[k];
                if (tidx < 0) continue;
                if (static_cast<size_t>(tidx) >= nTors) continue;
                dimsW.push_back(6 + static_cast<size_t>(tidx));
            }
            if (dimsW.empty()) continue;

            for (int t = 0; t < W_TRIES_PER_WINDOW && evals < MAX_EVALS; ++t) {
                auto x_try = propose_on_dims(dimsW, stepW);
                const double s_try = eval_full(x_try);
                if (accept(s_try, bestScore, temp)) {
                    x_cur = std::move(x_try);
                    bestScore = s_try;
        ...
        // stop / decay
        const double improve = score_before - bestScore;
        stepTR = std::max(STEP_TR_MIN, stepTR * STEP_DECAY);
        stepW  = std::max(STEP_W_MIN,  stepW  * STEP_DECAY);
        temp   = std::max(TEMP_MIN,    temp   * TEMP_DECAY);

        if (improve < 1e-3) break;
    }
...
) const
{
    ...
    // Tunables (edit here)
    // =========================
    const int    MAX_CYCLES            = 10;     // hard cap on TR->windows cycles
    const size_t MAX_WINDOWS_TO_USE    = 50;     // cap per cycle (runtime guard)
    const double MIN_IMPROVE_KCAL      = 1e-3;   // stop if a full cycle improves less than this

    // NM termination tolerances
    const double FTOL_TR               = 2e-4;
    const double FTOL_W                = 2e-4;

    // Per-coordinate xtol (overall xtol = per * sqrt(dim))
    const double XPER_TR               = 6e-4;
    const double XPER_W                = 6e-4;

    // Initial simplex sizes (normalized [0..1])
    const double EPS_TR0_SMALL         = 0.020;
    const double EPS_W0_SMALL          = 0.012;

    const double EPS_TR0_MANY          = 0.015;
    const double EPS_W0_MANY           = 0.010;

    // Minimum eps clamps
    const double EPS_TR_MIN            = 0.005;
    const double EPS_W_MIN             = 0.004;

    // Iteration budgets (decayed)
    const int    IT_TR0_SMALL          = 420;
    const int    IT_W0_SMALL           = 260;

    const int    IT_TR0_MANY           = 360;
    const int    IT_W0_MANY            = 220;
    ...
    // Optional small TR “re-seat” after torsion windows
    const bool   DO_POST_TR            = true;
    const double POST_TR_IT_FRAC       = 0.40;
    const double POST_TR_EPS_FRAC      = 0.75;

    // If no windows exist, fall back to chunking torsions [0..nTors) in groups of 4
    const bool   FALLBACK_CHUNK_WINDOWS = true;
    // =========================
    ...
    // Full objective on D-dim normalized vector
    auto eval_full = [&](const std::vector<double> &x_norm_in) -> double {
        std::vector<double> x_norm = x_norm_in;
        clamp01_inplace(x_norm);
    ...
    // Best-so-far normalized vector
    std::vector<double> x_cur(D, 0.5);
    double bestScore = eval_full(x_cur);

    // Subspace NM optimizer (optimize only idxs, keep others fixed at x_cur)
    auto optimize_subspace = [&](const std::vector<size_t>& idxs,
                                 double eps,
                                 int iters,
                                 double ftol,
        ...
        // simplex builder for K-dim
        auto make_simplex_K = [&](const std::vector<double>& x_start, double epsK) {
            std::vector<std::vector<double>> simplex(K + 1, x_start);
            for (size_t i = 0; i < K; ++i) {
                double v = x_start[i] + epsK;
                if (v > 1.0) v = x_start[i] - epsK;
                simplex[i + 1][i] = std::clamp(v, 0.0, 1.0);
            }
            return simplex;
        };

        // objective on subvector
        auto obj = [&](const std::vector<double>& x_sub_in) -> double {
            std::vector<double> x = x_cur;
            for (size_t i = 0; i < K; ++i) {
                x[idxs[i]] = std::clamp(x_sub_in[i], 0.0, 1.0);
        ...
        NelderMeadOptimizer nm(static_cast<int>(K));
        auto simplex = make_simplex_K(x0, eps);
        auto res = nm.optimize(obj, simplex, iters, ftol, xtol);
    ...
    // Windows source (torsion indices), x index = 6 + tidx
    const auto& windows_src = pre.ligand_score().ligand_torsion_windows;

    // Helper: get cycle params (eps/iters with decay)
    auto cycle_params = [&](int cyc, double& epsTR, int& itTR, double& epsW, int& itW) {
        const double epsTR0 = manyTors ? EPS_TR0_MANY : EPS_TR0_SMALL;
        const double epsW0  = manyTors ? EPS_W0_MANY  : EPS_W0_SMALL;
        const int itTR0     = manyTors ? IT_TR0_MANY   : IT_TR0_SMALL;
    ...
    while (cyc < MAX_CYCLES) {
        ...
        // (2) Windowed torsion micro-opts
        if (nTors > 0) {
            if (!windows_src.empty()) {
                const size_t nWinUse = std::min(MAX_WINDOWS_TO_USE, windows_src.size());

                for (size_t w = 0; w < nWinUse; ++w) {
                    const auto& win = windows_src[w];
            ...
            } else if (FALLBACK_CHUNK_WINDOWS) {
                // Fallback: chunk torsions into contiguous groups of 4
                for (size_t t0 = 0; t0 < nTors; t0 += 4) {
                    std::vector<size_t> idxW;
                    for (size_t t = t0; t < std::min(nTors, t0 + 4); ++t) {
                        idxW.push_back(6 + t);
                    }
                    optimize_subspace(
                        idxW,
                ...
                // If you prefer: do nothing when no windows exist
            }
        }

        // (3) Optional TR re-seat
        if (DO_POST_TR) {
            const int itTR2 = std::max(IT_TR_MIN, static_cast<int>(std::lround(itTR * POST_TR_IT_FRAC)));
            const double epsTR2 = std::max(EPS_TR_MIN, epsTR * POST_TR_EPS_FRAC);
        ...
        const double improve = score_before - bestScore;
        if (improve < MIN_IMPROVE_KCAL) break;
        ++cyc;
    }

    // Apply best pose to molecule
    restore_coords(conf, baseCoords);
    applyNormalizedSolution(pre, conf, x_cur, ini_trans_xyz, ini_rot_deg, ini_tors_deg);
...
) const
{
    ...
    const size_t MAX_WINDOWS_TO_USE    = 28;
    const double MIN_IMPROVE_KCAL      = 5e-3;

    // NM termination tolerances
    const double FTOL_T                = 2e-4;
    const double FTOL_R                = 2e-4;
    const double FTOL_W                = 2e-4;

    // Per-coordinate xtol (overall xtol = per * sqrt(dim))
    const double XPER_T                = 6e-4;
    const double XPER_R                = 1.5e-4;
    const double XPER_W                = 6e-4;

    // Initial simplex sizes (normalized [0..1])
    const double EPS_T0_SMALL          = 0.020;
    const double EPS_W0_SMALL          = 0.012;

    const double EPS_T0_MANY           = 0.015;
    const double EPS_W0_MANY           = 0.010;

    // Rotation eps relative to translation eps
    const double EPS_R_SCALE           = 0.25;    // 0.02 -> 0.005 (~1.8°)
    // Minimum eps clamps
    const double EPS_T_MIN             = 0.005;
    const double EPS_R_MIN             = 0.00125; // EPS_T_MIN * EPS_R_SCALE
    const double EPS_W_MIN             = 0.004;

    // Iteration budgets (decayed)
    const int    IT_T0_SMALL           = 420;
    const int    IT_W0_SMALL           = 260;

    const int    IT_T0_MANY            = 360;
    const int    IT_W0_MANY            = 220;

    const int    IT_T_MIN              = 120;
    const int    IT_R_MIN              = 120;
    const int    IT_W_MIN              = 90;
    ...
    // Optional small TR “re-seat” after torsion windows
    const bool   DO_POST_TR            = false;
    const double POST_TR_IT_FRAC       = 0.40;
    const double POST_TR_EPS_FRAC      = 0.75;

    const bool   FALLBACK_CHUNK_WINDOWS = true;

    // Tail cut-off: if we're already at min eps/iters and not improving, bail.
    const double TAIL_IMPROVE_FACTOR   = 5.0; // break if improve < factor * MIN_IMPROVE_KCAL
    // =========================
    ...
    // Full objective on D-dim normalized vector.
    // IMPORTANT: assumes x_norm already clamped in [0,1].
    //
    // Speedup #4 (partial, without refactoring scorer/apply):
    //   - no vector copies/clamp here
    //   - duplicate-eval avoidance is done in optimize_subspace()
    ...
    // Subspace NM optimizer (optimize only idxs, keep others fixed at x_cur)
    // Speedup #3:
    //   - objective modifies x_cur in-place and restores it (no "std::vector<double> x = x_cur" per eval)
    //   - avoids re-scoring the same clamped point repeatedly (common near bounds in the tail)
    auto optimize_subspace = [&](const std::vector<size_t>& idxs,
                                 double eps,
                                 int iters,
                                 double ftol,
        ...
        // scratch buffers reused across objective calls (no per-call allocations)
        std::vector<double> old_vals(K, 0.0);
        std::vector<double> last_sub(K, 0.0);
        bool have_last = false;
        double last_val = 0.0;

        auto obj = [&](const std::vector<double>& x_sub_in) -> double {
            // save old
            for (size_t i = 0; i < K; ++i) old_vals[i] = x_cur[idxs[i]];

            // write clamped new subspace values into x_cur
            bool same_as_last = have_last;
            for (size_t i = 0; i < K; ++i) {
                const double v = std::clamp(x_sub_in[i], 0.0, 1.0);
                x_cur[idxs[i]] = v;
                if (same_as_last && v != last_sub[i]) same_as_last = false;
            }

            // If optimizer proposed something that clamps to the same point as last time, skip heavy eval.
            if (same_as_last) {
                // restore x_cur
                for (size_t i = 0; i < K; ++i) x_cur[idxs[i]] = old_vals[i];
                return last_val;
            }

            const double val = eval_full(x_cur);

            // update cache
            for (size_t i = 0; i < K; ++i) last_sub[i] = x_cur[idxs[i]];
            have_last = true;
            last_val = val;
    ...
    // Split TR indices: translations (0..2), rotations (3..5)
    const std::vector<size_t> idxT = {0,1,2};
    const std::vector<size_t> idxR = {3,4,5};

    // Windows source (torsion indices), x index = 6 + tidx
    const auto& windows_src = pre.ligand_score().ligand_torsion_windows;

    // Helper: get cycle params (eps/iters with decay)
    auto cycle_params = [&](int cyc,
                            double& epsT, int& itT,
                            double& epsR, int& itR,
                            double& epsW, int& itW) {
        const double epsT0 = manyTors ? EPS_T0_MANY : EPS_T0_SMALL;
        const double epsW0 = manyTors ? EPS_W0_MANY : EPS_W0_SMALL;
        const int itT0     = manyTors ? IT_T0_MANY   : IT_T0_SMALL;
        const int itW0     = manyTors ? IT_W0_MANY   : IT_W0_SMALL;

        const double powE = std::pow(EPS_DECAY, static_cast<double>(cyc));
        const double powI = std::pow(IT_DECAY,  static_cast<double>(cyc));

        epsT = std::max(EPS_T_MIN, epsT0 * powE);
        epsR = std::max(EPS_R_MIN, (epsT0 * EPS_R_SCALE) * powE);
        epsW = std::max(EPS_W_MIN, epsW0 * powE);

        itT  = std::max(IT_T_MIN, static_cast<int>(std::lround(itT0 * powI)));
        itR  = std::max(IT_R_MIN, static_cast<int>(std::lround(itT0 * powI)));
        itW  = std::max(IT_W_MIN, static_cast<int>(std::lround(itW0 * powI)));
    };
    ...
    while (cyc < MAX_CYCLES) {
        ...
        double epsT, epsR, epsW;
        int itT, itR, itW;
        cycle_params(cyc, epsT, itT, epsR, itR, epsW, itW);

        // Speedup #1: identify tail state (min eps + min iters)
        const bool at_min =
            (epsT <= EPS_T_MIN + 1e-15) &&
            (epsR <= EPS_R_MIN + 1e-15) &&
            (epsW <= EPS_W_MIN + 1e-15) &&
            (itT == IT_T_MIN) &&
            (itR == IT_R_MIN) &&
            (itW == IT_W_MIN);

        // (1a) Translation micro-opt
        optimize_subspace(
            idxT,
            epsT,
            itT,
            FTOL_T,
            xtol_from_percoord(XPER_T, idxT.size())
        );

        // (1b) Rotation micro-opt
        optimize_subspace(
            idxR,
            epsR,
            itR,
            FTOL_R,
            xtol_from_percoord(XPER_R, idxR.size())
        );
        ...
        // (3) Optional TR re-seat (reduced budgets)
        if (DO_POST_TR) {
            const int itT2 = std::max(IT_T_MIN, static_cast<int>(std::lround(itT * POST_TR_IT_FRAC)));
            const int itR2 = std::max(IT_R_MIN, static_cast<int>(std::lround(itR * POST_TR_IT_FRAC)));
            const double epsT2 = std::max(EPS_T_MIN, epsT * POST_TR_EPS_FRAC);
            const double epsR2 = std::max(EPS_R_MIN, epsR * POST_TR_EPS_FRAC);
        ...
        // Speedup #1: earlier tail stop
        if (improve < MIN_IMPROVE_KCAL) break;
        if (at_min && cyc >= 2 && improve < (TAIL_IMPROVE_FACTOR * MIN_IMPROVE_KCAL)) break;
...
) const
{
    ...
    // Normalized step sizes in [0..1] space (split TR into trans vs rot)
    double stepTrans0 = 0.010;   // ~0.04 Å per axis
    double stepRot0   = 0.010;   // ~3.6° per axis
    double stepW0     = 0.030;   // ~10.8° per torsion

    const double STEP_TRANS_MIN = 0.006;
    const double STEP_ROT_MIN   = 0.006;
    const double STEP_W_MIN     = 0.005;
    const double STEP_DECAY     = 0.75;

    const bool   ALLOW_UPHILL = false;
    double       TEMP0        = 0.25;
    const double TEMP_MIN     = 0.03;
    const double TEMP_DECAY   = 0.80;
    ...
    const size_t MAX_WINDOWS_TO_USE = 28;
    const bool   FALLBACK_CHUNK_WINDOWS = true;
    constexpr size_t FALLBACK_CHUNK = 4;
    // =========================
    ...
    auto eval_full = [&](const std::vector<double> &x_norm_in) -> double {
        ...
        restore_coords(conf, baseCoords);
        applyNormalizedSolution(pre, conf, x, ini_trans_xyz, ini_rot_deg, ini_tors_deg);
        ++evals;
        return scorer.score(conf, rep_max, map_score_function);
    };
    ...
    // -------------------------
    const auto &wins_src = pre.ligand_score().ligand_torsion_windows; // std::vector<std::vector<int>>
    std::vector<std::vector<int>> wins_use;

    if (!wins_src.empty()) {
        const size_t nUse = std::min(MAX_WINDOWS_TO_USE, wins_src.size());
        wins_use.assign(wins_src.begin(), wins_src.begin() + nUse);
    } else if (FALLBACK_CHUNK_WINDOWS && nTors > 0) {
        for (size_t t0 = 0; t0 < nTors; t0 += FALLBACK_CHUNK) {
            std::vector<int> win;
            win.reserve(std::min(FALLBACK_CHUNK, nTors - t0));
            for (size_t t = t0; t < std::min(nTors, t0 + FALLBACK_CHUNK); ++t) {
                win.push_back(static_cast<int>(t));
            }
            wins_use.push_back(std::move(win));
            if (wins_use.size() >= MAX_WINDOWS_TO_USE) break;
        }
    }
    ...
    auto propose_with_steps = [&](const std::vector<size_t> &dims,
                                  const std::vector<double> &steps) -> std::vector<double> {
        std::vector<double> x = x_cur;
        for (size_t i = 0; i < dims.size(); ++i) {
            const size_t d = dims[i];
            x[d] = clamp01(x[d] + steps[i] * U(rng));
        }
        return x;
    };
    ...
    double stepTrans = stepTrans0;
    double stepRot   = stepRot0;
    double stepW     = stepW0;
    double temp      = TEMP0;
    ...
    for (int cyc = 0; cyc < MAX_CYCLES && evals < MAX_EVALS; ++cyc) {
        ...
        // --- (1) TR hillclimb ---
        const std::vector<double> stepsTR = {
            stepTrans, stepTrans, stepTrans,
            stepRot,   stepRot,   stepRot
        };

        for (int t = 0; t < TR_TRIES_PER_CYCLE && evals < MAX_EVALS; ++t) {
            auto x_try = propose_with_steps(dimsTR, stepsTR);
            const double s_try = eval_full(x_try);
            if (accept(s_try, bestScore, temp)) {
                x_cur = std::move(x_try);
                bestScore = s_try;
        ...
        // --- (2) Window torsions hillclimb ---
        if (nTors > 0 && !wins_use.empty()) {
            for (const auto &win : wins_use) {
                if (evals >= MAX_EVALS) break;
                if (win.empty()) continue;

                std::vector<size_t> dimsW;
                dimsW.reserve(win.size());

                for (int tidx : win) {
                    if (tidx < 0) continue; // if you still allow padding
                    const size_t ut = static_cast<size_t>(tidx);
                    if (ut >= nTors) continue;
                    dimsW.push_back(6 + ut);
                }

                std::sort(dimsW.begin(), dimsW.end());
                dimsW.erase(std::unique(dimsW.begin(), dimsW.end()), dimsW.end());
                if (dimsW.empty()) continue;

                std::vector<double> stepsW(dimsW.size(), stepW);

                for (int t = 0; t < W_TRIES_PER_WINDOW && evals < MAX_EVALS; ++t) {
                    auto x_try = propose_with_steps(dimsW, stepsW);
                    const double s_try = eval_full(x_try);
                    if (accept(s_try, bestScore, temp)) {
                        x_cur = std::move(x_try);
                        bestScore = s_try;
        ...
        // Stop if nothing improved this cycle
        if ((score_before - bestScore) <= 1e-6) {
            if (!ALLOW_UPHILL) break;
        }

        // Decay steps/temperature
        stepTrans = std::max(STEP_TRANS_MIN, stepTrans * STEP_DECAY);
        stepRot   = std::max(STEP_ROT_MIN,   stepRot   * STEP_DECAY);
        stepW     = std::max(STEP_W_MIN,     stepW     * STEP_DECAY);
        temp      = std::max(TEMP_MIN,       temp      * TEMP_DECAY);

        if ((score_before - bestScore) < 1e-3) break;
    }
