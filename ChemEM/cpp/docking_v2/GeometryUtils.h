...
namespace GeometryUtils {
    ...
    // Trilinear interpolation returning the value AND the analytic spatial gradient d/d(x,y,z).
    // Same 8 corners as trilinear_sample; the interpolation is trilinear in the fractional coords
    // so the gradient is closed-form. Out-of-bounds -> value 0, gradient 0 (matches trilinear_sample).
    inline double trilinear_sample_grad(
        const std::vector<double>& grid,
        const Eigen::RowVector3d& pos,
        const Eigen::RowVector3d& origin,
        const Eigen::RowVector3d& spacing,
        int nz, int ny, int nx,
        Eigen::RowVector3d& grad)          // out: d(value)/d(x,y,z), world units
    {
        grad.setZero();
        const double gx = (pos.x() - origin.x()) / spacing[0];
        const double gy = (pos.y() - origin.y()) / spacing[1];
        const double gz = (pos.z() - origin.z()) / spacing[2];

        const int x0 = static_cast<int>(std::floor(gx));
        const int y0 = static_cast<int>(std::floor(gy));
        const int z0 = static_cast<int>(std::floor(gz));

        if (x0 < 0 || x0 + 1 >= nx || y0 < 0 || y0 + 1 >= ny || z0 < 0 || z0 + 1 >= nz)
            return 0.0;

        const double tx = gx - x0, ty = gy - y0, tz = gz - z0;
        const int x1 = x0 + 1, y1 = y0 + 1, z1 = z0 + 1;

        const double v000 = grid[idx3D(z0, y0, x0, nz, ny, nx)];
        const double v100 = grid[idx3D(z0, y0, x1, nz, ny, nx)];
        const double v010 = grid[idx3D(z0, y1, x0, nz, ny, nx)];
        const double v110 = grid[idx3D(z0, y1, x1, nz, ny, nx)];
        const double v001 = grid[idx3D(z1, y0, x0, nz, ny, nx)];
        const double v101 = grid[idx3D(z1, y0, x1, nz, ny, nx)];
        const double v011 = grid[idx3D(z1, y1, x0, nz, ny, nx)];
        const double v111 = grid[idx3D(z1, y1, x1, nz, ny, nx)];

        const double omx = 1.0 - tx, omy = 1.0 - ty, omz = 1.0 - tz;

        // value
        const double vx00 = v000 * omx + v100 * tx;
        const double vx10 = v010 * omx + v110 * tx;
        const double vx01 = v001 * omx + v101 * tx;
        const double vx11 = v011 * omx + v111 * tx;
        const double vxy0 = vx00 * omy + vx10 * ty;
        const double vxy1 = vx01 * omy + vx11 * ty;
        const double value = vxy0 * omz + vxy1 * tz;

        // d(value)/d(fractional coord), then chain to world units via /spacing
        const double dvdtx = ((v100 - v000) * omy * omz + (v110 - v010) * ty * omz
                            + (v101 - v001) * omy * tz + (v111 - v011) * ty * tz);
        const double dvdty = ((v010 - v000) * omx * omz + (v110 - v100) * tx * omz
                            + (v011 - v001) * omx * tz + (v111 - v101) * tx * tz);
        const double dvdtz = ((v001 - v000) * omx * omy + (v101 - v100) * tx * omy
                            + (v011 - v010) * omx * ty + (v111 - v110) * tx * ty);

        grad.x() = dvdtx / spacing[0];
        grad.y() = dvdty / spacing[1];
        grad.z() = dvdtz / spacing[2];
        return value;
    }
