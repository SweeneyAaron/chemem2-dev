...
// Per-ligand-atom vdW affinity grids for the fast (grid-approximate) sampling score. One set per
// heavy atom; the protein's summed vdW contribution to a probe with that atom's params is baked in
// per grid point, split into attr/rep/clash so apply_weights' split channels are honoured. Built
// once per dock only when config.fast_sample is set (see PreComputedData::build_vdw_grids).
struct VdwAtomGrids {
    GridData attr;   // r>=2 pairs with val<=0 (attractive vdW)
    GridData rep;    // r>=2 pairs with val>0  (soft-capped repulsion)
    GridData clash;  // r<2 pairs (smoothstep clash penalty)
};
...
struct AlgorithmConfig {
    ...
    bool fast_sample = false;   // build + use per-atom vdW grids to score the ant-sampling stage
    // Seed the final polish from the REFINED conformer instead of re-deriving the pose from the
    // seed's discrete solution. Without this the R2 refinement is used for ranking and then
    // discarded: refinePoseGrid leaves discSol untouched, and the non-fast path writes back a
    // 30 deg-quantised approximation, so the polish restarts from a coarser pose than the one
    // that was scored. Default false = previous behaviour.
    bool polish_from_refined = false;
    // Which density field the map_lookup term samples.
    //   0 = ccc0 smoothed (local CC vs a Gaussian template)
    //   1 = ccc1 gradient-magnitude CC
    //   2 = ccc2 Laplacian CC
    //   3 = raw cropped density   <-- best separator measured on 9DMU/GAD
    //   4 = ccc0 + ccc1 + ccc2 (what score_sci_fast averages)
    //   5 = raw density normalised by its own max -- same shape as 3 but amplitude-independent,
    //       so a weight tuned on one map transfers to another
    // Measured native-vs-decoy separation over 21 poses: raw 1.87x (|rho| 0.79) versus
    // 1.06-1.12x (|rho| 0.42-0.53) for every CCC variant -- the CCC maps are clamped to [0,1]
    // with ~90% of voxels nonzero, so they are nearly flat and discriminate poorly.
    int map_lookup_channel = 0;
    // Torsion-only L-BFGS iterations run after the grid refine (0 = off, previous behaviour).
    // Classify an H-bond by GEOMETRY (mask + roles + angle > 110 deg + distance) rather than by
    // the sign of the H-bond polynomial, purely so a short-but-real H-bond still marks its ligand
    // atom satisfied. nonbond_rep is unaffected. false = previous behaviour.
    bool hbond_geometric_gate = false;
    int torsion_refine_steps = 0;
    int grid_min_steps = 25;    // analytic grid L-BFGS iterations in the R2 refine (fast_sample)
    double pose_min_rmsd = 0.0; // finer output/dedup RMSD for returned modes (0 -> use rms_cutoff)
    double energy_range = 0.0;  // return only modes within this of the best score (0 -> disabled)
};
...
public:
    ...
    const std::vector<VdwAtomGrids>& vdw_grids() const { return m_vdw_grids ; }
    // TEMP recall diagnostic: native heavy-atom coords (docked-ligand order) + whether they're set
    const MatX3d& reference_heavy() const { return m_reference_heavy ; }
    ...
    // 1/max of the raw density grid (1.0 if empty/degenerate). The raw grid carries the map's
    // own amplitude scale -- measured max 13.41 on 9DMU vs 1.000 for every CCC channel -- so a
    // map_lookup weight tuned on one map does not transfer. Channel 5 divides it out.
    double density_grid_norm() const { return m_density_grid_norm; }
...
private: 
    ...
    std::vector<VdwAtomGrids> m_vdw_grids;   // per-heavy-atom vdW affinity grids (fast_sample only)
    MatX3d m_reference_heavy;                // TEMP recall diagnostic: native heavy-atom coords
    ...
    double m_density_grid_norm = 1.0;
    GridData m_sci_grid;
    ...
    void build_vdw_grids();   // precompute per-atom vdW affinity grids over the electro-grid box
