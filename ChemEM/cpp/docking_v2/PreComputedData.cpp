...
PreComputedData::PreComputedData(py::object py_pc){
     ...
     */
     ...
     try { 
         ...
         // inner -> ant sampling + inner Nelder-Mead refine; outer -> final polish,
         // i.e. the score the returned poses are ranked by.
         ...
         // optional (older py_pc dicts lack it -> stays false / baseline behaviour)
         if (py::hasattr(py_pc, "fast_sample"))
             m_config.fast_sample = py_pc.attr("fast_sample").cast<bool>();
         if (py::hasattr(py_pc, "polish_from_refined"))
             m_config.polish_from_refined = py_pc.attr("polish_from_refined").cast<bool>();
         if (py::hasattr(py_pc, "map_lookup_channel"))
             m_config.map_lookup_channel = py_pc.attr("map_lookup_channel").cast<int>();
         if (py::hasattr(py_pc, "hbond_geometric_gate"))
             m_config.hbond_geometric_gate = py_pc.attr("hbond_geometric_gate").cast<bool>();
         if (py::hasattr(py_pc, "torsion_refine_steps"))
             m_config.torsion_refine_steps = py_pc.attr("torsion_refine_steps").cast<int>();
         if (py::hasattr(py_pc, "grid_min_steps"))
             m_config.grid_min_steps = py_pc.attr("grid_min_steps").cast<int>();
         if (py::hasattr(py_pc, "pose_min_rmsd"))
             m_config.pose_min_rmsd = py_pc.attr("pose_min_rmsd").cast<double>();
         if (py::hasattr(py_pc, "energy_range"))
             m_config.energy_range = py_pc.attr("energy_range").cast<double>();
         // TEMP recall diagnostic: native heavy-atom coords (docked-ligand order)
         if (py::hasattr(py_pc, "reference_heavy_coords")
                 && !py_pc.attr("reference_heavy_coords").is_none()) {
             m_reference_heavy = py_pc.attr("reference_heavy_coords").cast<MatX3d>();
             m_has_reference = (m_reference_heavy.rows() > 0);
         }
        ...
        if (!ds_obj.is_none() && (!m_config.no_map)) {
             ...
             try {
                 ...
                 // scale factor for map_lookup_channel 5 (amplitude-independent raw density)
                 if (!m_density_grid.data.empty()) {
                     double mx = 0.0;
                     for (double v : m_density_grid.data) mx = std::max(mx, v);
                     m_density_grid_norm = (mx > 1e-12) ? (1.0 / mx) : 1.0;
                 }
             } catch (const std::exception &e) {
        ...
        // Fast-sampling affinity grids: precompute the protein's vdW contribution per heavy atom
        // over the binding-site box, so the ant-sampling stage scores by trilinear interpolation
        // instead of the protein-neighbour loop. Only when opted in (keeps baseline untouched).
        if (m_config.fast_sample) {
            try {
                build_vdw_grids();
            } catch (const std::exception &e) {
                throw std::runtime_error("[Error] PreComputedData build_vdw_grids failed: " + std::string(e.what()));
            }
        }
...
// Precompute one attr/rep/clash affinity grid per ligand HEAVY atom. For each grid point (reusing
// the electro grid's box geometry so trilinear_sample maps identically) we query the protein spatial
// hash once, then accumulate — for every heavy atom — the SAME per-pair vdW form + soft-cap + r<2
// clash used in compute_nonbond_hbond_salt (ScoringFunctions.cpp). Directional H-bond / salt /
// aromatic are intentionally dropped (recovered by the full score in refine). The soft-cap uses a
// FIXED rep_max (config.repCap1) since the grid is prebuilt.
void PreComputedData::build_vdw_grids() {
    const GridData &box = m_electro_grid;               // reuse box geometry (origin/apix/dims)
    const int nx = box.nx, ny = box.ny, nz = box.nz;
    if (nx <= 1 || ny <= 1 || nz <= 1)
        throw std::runtime_error("electro grid box not loaded before build_vdw_grids");

    const MatrixXd &pos = m_protein_data.positions;
    const MatrixXd &A = m_hbond_score_data.A_values;    // (n_prot x n_lig)
    const MatrixXd &B = m_hbond_score_data.B_values;
    const MatrixXd &C = m_hbond_score_data.C_values;
    const SpatialHash &hash = m_config.prot_hash;
    const double cutoff = m_config.interaction_cutoff;
    const double cutoff_sq = cutoff * cutoff;
    const double REP = m_config.repCap1;                // fixed soft-cap for the prebuilt grid
    const int n_heavy = static_cast<int>(m_ligand_data.heavy_end_idx) + 1;
    const std::size_t npts = static_cast<std::size_t>(nx) * ny * nz;

    m_vdw_grids.assign(n_heavy, VdwAtomGrids{});
    for (int ai = 0; ai < n_heavy; ++ai) {
        for (GridData *g : {&m_vdw_grids[ai].attr, &m_vdw_grids[ai].rep, &m_vdw_grids[ai].clash}) {
            g->origin = box.origin; g->apix = box.apix;
            g->nx = nx; g->ny = ny; g->nz = nz;
            g->data.assign(npts, 0.0);
        }
    }
    ...
    #pragma omp parallel
    {
        ...
        for (int y = 0; y < ny; ++y)
        for (int x = 0; x < nx; ++x) {
            const Eigen::RowVector3d gp(box.origin[0] + x * box.apix[0],
                                        box.origin[1] + y * box.apix[1],
                                        box.origin[2] + z * box.apix[2]);
            neigh.clear();
            hash.query(gp, cutoff, pos, neigh);
            if (neigh.empty()) continue;
            const std::size_t gi = static_cast<std::size_t>(z * ny + y) * nx + x;

            for (int ai = 0; ai < n_heavy; ++ai) {
                double a_sum = 0.0, r_sum = 0.0, c_sum = 0.0;
                for (int pi : neigh) {
                    const double dx = gp[0] - pos(pi, 0);
                    const double dy = gp[1] - pos(pi, 1);
                    const double dz = gp[2] - pos(pi, 2);
                    ...
                    if (r < 2.0) {
                        ...
                        c_sum += REP * (1.0 + f);               // clash channel (matches r<2 branch)
                    } else {
                        const double r6 = r2 * r2 * r2;
                        const double val = A(pi, ai) * std::exp(-B(pi, ai) * r) - C(pi, ai) / r6;
                        if (val <= 0.0) a_sum += val;           // attractive
                        else            r_sum += REP * std::tanh(val / REP);  // soft-capped repulsion
                    }
                }
                m_vdw_grids[ai].attr.data[gi]  = a_sum;
                m_vdw_grids[ai].rep.data[gi]   = r_sum;
                m_vdw_grids[ai].clash.data[gi] = c_sum;
            }
        }
    }
}
