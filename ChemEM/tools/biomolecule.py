...
# -------------------------
def _to_angstrom_positions(positions) -> np.ndarray:
...
def _max_namewise_delta(posA, idxsA, posB, idxsB) -> float:
    PA = posA[idxsA]
    PB = posB[idxsB]
    return max(np.linalg.norm(PB - p, axis=1).min() for p in PA)

def _residues_match_by_backbone(rA, rB, posA, posB, tol_ang: float = 1e-5) -> bool:
    mapA = _atomname_to_idx(rA)
    mapB = _atomname_to_idx(rB)
    common_bb = [n for n in ("N", "CA", "C", "O") if (n in mapA and n in mapB)]
    if len(common_bb) < 2:
        return False
    for name in common_bb:
        d = _max_namewise_delta(posA, mapA[name], posB, mapB[name])
        if d > tol_ang:
            return False
    return True
...
def build_residue_map_by_positions(orig_topology, orig_positions,
    ...
    """
    posA = _to_angstrom_positions(orig_positions)
    posB = _to_angstrom_positions(final_positions)
    ...
    for rA in orig_residues:
        keyA = (_chain_id(rA), str(rA.id))
        candidates = buckets.get(rA.name, [])

        hit = None
        for rB in candidates:
            if _residues_match_by_backbone(rA, rB, posA, posB, tol_ang=1e-5):
                hit = rB
                break

        if hit is not None:
            res_map[keyA] = (_chain_id(hit), str(hit.id))
