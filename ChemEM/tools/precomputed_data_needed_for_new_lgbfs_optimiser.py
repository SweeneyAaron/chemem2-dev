...
def _downstream_heavy_count(mol: Chem.Mol, B: int, C: int) -> int:
    """Count heavy atoms reachable from C without crossing back to B."""
    seen = {B}
    q = deque([C])
    cnt = 0
    while q:
        ...
        if u in seen:
            continue
        seen.add(u)
        if mol.GetAtomWithIdx(u).GetAtomicNum() != 1:
            cnt += 1
...
def torsion_impacts(mol: Chem.Mol, torsions):
    """Return list impact[tidx] = downstream heavy count."""
    imp = []
    for (A,B,C,D) in torsions:
        # donor-H torsions typically have tiny impact; keep them low
        if mol.GetAtomWithIdx(D).GetAtomicNum() == 1:
            imp.append(1)
        else:
            imp.append(_downstream_heavy_count(mol, B, C))
    return imp

def order_windows_by_impact(mol: Chem.Mol, torsions, tail_windows_idx, *, mode="max"):
    """
    mode:
      - 'max': window score = max impact among torsions in window (good default)
      - 'sum': window score = sum impacts (pushes longer windows first)
    """
    imp = torsion_impacts(mol, torsions)

    scored = []
    for w, win in enumerate(tail_windows_idx):
        vals = [imp[t] for t in win if t >= 0]
        if not vals:
            continue
        score = max(vals) if mode == "max" else sum(vals)
        scored.append((score, w))

    scored.sort(reverse=True)  # highest impact first
    return [tail_windows_idx[w] for _, w in scored]

def order_torsions_within_window(mol: Chem.Mol, torsions, win_idx):
    """Optional: reorder torsions inside a window by impact (high->low)."""
    imp = torsion_impacts(mol, torsions)
    return sorted(win_idx, key=lambda t: imp[t], reverse=True)
...
def build_tail_torsion_windows(
    mol: Chem.Mol,
    rooted_torsions: List[Torsion],
    root_atom: int,
    *,
    window: int = 4,
    min_path_tors: int = 2,                 # <-- FIX (was effectively breaking you)
    max_blocks_per_tail: Optional[int] = None,
    include_donor_h_windows: bool = True,  # optional
) -> List[List[int]]:
    """
    Return tail torsion windows as lists of indices into `rooted_torsions`.

    - Builds heavy-atom BFS tree from root_atom.
    - Finds terminal heavy atoms.
    - For each terminal, walks back to root and collects torsions along the path.
    - Emits sliding windows (distal -> proximal).
    """

    def heavy_neighbors(aidx: int) -> List[int]:
        a = mol.GetAtomWithIdx(aidx)
        return [n.GetIdx() for n in a.GetNeighbors() if n.GetAtomicNum() != 1]

    def heavy_degree(aidx: int) -> int:
        return len(heavy_neighbors(aidx))

    n = mol.GetNumAtoms()
    parent = [-1] * n
    depth  = [-1] * n

    # ensure root is heavy
    if mol.GetAtomWithIdx(root_atom).GetAtomicNum() == 1:
        for a in mol.GetAtoms():
            if a.GetAtomicNum() != 1:
                root_atom = a.GetIdx()
                break

    # BFS over heavy atoms
    q = deque([root_atom])
    parent[root_atom] = root_atom
    depth[root_atom]  = 0
    while q:
        u = q.popleft()
        for v in heavy_neighbors(u):
            if depth[v] != -1:
                continue
            depth[v] = depth[u] + 1
            parent[v] = u
    ...
    # Map directed edge (up, down) -> torsion index
    # Robust: decide direction using BFS depth rather than trusting (B,C) always matches.
    edge_to_tidx: Dict[Tuple[int, int], int] = {}
    donor_h_idxs: List[int] = []

    for t_idx, (A, B, C, D) in enumerate(rooted_torsions):
        if mol.GetAtomWithIdx(D).GetAtomicNum() == 1:
            donor_h_idxs.append(t_idx)
            continue

        # skip if disconnected in BFS
        if depth[B] < 0 or depth[C] < 0:
            continue

        up, down = (B, C) if depth[B] < depth[C] else (C, B)
        edge_to_tidx[(up, down)] = t_idx

    # terminal heavy atoms (excluding root)
    terminals = [
        a.GetIdx()
        for a in mol.GetAtoms()
        if a.GetAtomicNum() != 1 and a.GetIdx() != root_atom and heavy_degree(a.GetIdx()) == 1
    ]
    ...
    for term in terminals:
        ...
        # walk terminal -> root
        while node != root_atom and parent[node] != -1 and parent[node] != node:
            p = parent[node]
            tidx = edge_to_tidx.get((p, node))
            if tidx is not None:
                path_tors.append(tidx)
            node = p

        if len(path_tors) < int(min_path_tors):
            continue

        # emit windows (distal -> proximal)
        if len(path_tors) <= window:
            blocks = [path_tors]
        else:
            blocks = [path_tors[i:i+window] for i in range(0, len(path_tors) - window + 1)]

        if max_blocks_per_tail is not None:
            blocks = blocks[:max_blocks_per_tail]
    ...
    # optionally add donor-H torsions as singleton windows
    if include_donor_h_windows:
        for t_idx in donor_h_idxs:
            key = (t_idx,)
            if key not in seen:
                seen.add(key)
                windows.append([t_idx])

    return windows


def windows_to_torsions(rooted_torsions: List[Torsion], windows: List[List[int]]) -> List[List[Torsion]]:
    """Convert index windows into torsion tuple windows."""
    return [[rooted_torsions[i] for i in w] for w in windows]
...
class PreCompDataLigand:
    ...
    def __init__(self, ligand, platform, flexible_rings = False):
        ...
        torsion_lists = get_torsion_lists(ligand.mol)
        print('[DEBUG] old torsions: ', torsion_lists)
        '''
        root, self.torsion_lists = get_rooted_torsion_lists(
                            ligand.mol,  # must have explicit H for donor torsions
                            only_h_moves_on_rotation_fn=only_h_moves_on_rotation,
                        )
        '''
       
        #trying windowed torsions
        root, tors = get_rooted_torsion_lists(ligand.mol, only_h_moves_on_rotation_fn=only_h_moves_on_rotation)
        self.torsion_lists = tors
        print('[ROOT] Root atom idx', root)
        for num, i in enumerate(ligand.mol.GetAtoms()):
            print(num, i.GetSymbol())
        
        print('[DEBUG] new torsions: ', self.torsion_lists )
        
        tail_windows_idx = build_tail_torsion_windows(ligand.mol, tors, root, window=4, max_blocks_per_tail=20)
        self.tail_windows_idx = order_windows_by_impact(ligand.mol, tors, tail_windows_idx, mode="max")
        
        
        tail_windows = windows_to_torsions(tors, self.tail_windows_idx)
        print('[DEBUG] tail_window_index', self.tail_windows_idx)
        print('[DEBUG] tail_windows' ,tail_windows )
        
        #for win in self.tail_windows_idx:
        #    for tidx in win:
        #        assert 0 <= tidx < len(self.torsion_lists)
        ...
        print('[ACTUAL TORSIONS] new torsions: ', self.ligand_torsion_idxs)
        self.ligand_torsion_scores = [i[2] for i in self.ligand_torsion_profile]
        # new_torsions: iterable of (A,B,C,D) from your flexible ring fragmentation
        # goal: add only if central bond (B,C) not already present in torsion_lists,
        # and add ALL newly-added torsions as ONE group (one window).
        
        bond_keys = {tuple(sorted((b, c))) for (a, b, c, d) in self.torsion_lists}
        
        self._torsion_lists = self.torsion_lists.copy()
        
        to_add = []
        for tor in new_torsions:
            tor = list(tor)
            key = tuple(sorted((tor[1], tor[2])))
            if key in bond_keys:
                continue
            bond_keys.add(key)
            to_add.append(tor)
        ...
        if to_add:
            start = len(self.ligand_torsion_idxs)
            self.ligand_torsion_idxs.extend(to_add)
        
            ring_group = list(range(start, start + len(to_add)))
            
            
            # Make sure this group is always evaluated together (and early)
            self.tail_windows_idx.insert(0, ring_group)
        
        print('[DEBUG] flexible ring torsions: ')
...
class PreCompDataProtein:
    def __init__(self,
        ...
        self.rms_cutoff =system.options.cluster_docking #2.0
        ...
        #-----binding site data------
        #debug
        #dist = EMMap(binding_site.origin, binding_site.apix, binding_site.distance_map > probe_radius, 0.0)
        
        #di2 = dist.resample_to_apix(1.5)
        #binding_site.distance_map = di2.density_map 
        #binding_site.apix = di2.apix
        
        #self.translation_points, self.adjacency = get_valid_points_and_adjacency(binding_site.distance_map,
        #                                                              connectivity=26.0)
...
def precompute_score_maps(
    ...
    origin, apix, r,
    # new knobs:
    blur_sigma_A=0,        # Å; increase (e.g. 2–3 Å) if maps are very sparse
    blur_truncate=3.0,
...
def get_rooted_torsion_lists(
    mol: Chem.Mol,
    *,
    root_atom: Optional[int] = None,
    include_rotatable_torsions: bool = True,
    include_donor_h_torsions: bool = True,
    only_h_moves_on_rotation_fn: Optional[Callable[[Chem.Mol, Torsion], bool]] = None,
    rot_smarts: str = "[!$(*#*)&!D1]-&!@[!$(*#*)&!D1]",
) -> Tuple[int, List[Torsion]]:
    """
    Build a deterministic, rooted torsion list for docking.

    Returns:
      (root_atom_idx, torsions)

    Conventions:
      - For rotatable-bond torsions: (A,B,C,D) with B–C rotatable and C is "downstream" (farther from root).
      - For donor-H torsions: (A,B,C,D) with B=pivot, C=donor, D=explicit H on donor.
        With RDKit SetDihedralDeg, choosing D as H means only that H should move.
    """

    def heavy_degree(a: Chem.Atom) -> int:
        return sum(1 for n in a.GetNeighbors() if n.GetAtomicNum() != 1)
    '''
    def choose_root_atom(m: Chem.Mol) -> int:
        """
        Pick the heavy atom with minimum sum of topological distances to all other heavy atoms.
        Tie-break: prefer ring atoms, then higher heavy-degree, then higher atomic number.
        """
        heavy_idxs = [a.GetIdx() for a in m.GetAtoms() if a.GetAtomicNum() != 1]
        if not heavy_idxs:
            return 0
    
        # RDKit topological distance matrix (shortest-path distances in bonds)
        dm = rdmolops.GetDistanceMatrix(m)
    
        def heavy_degree_idx(idx: int) -> int:
            a = m.GetAtomWithIdx(idx)
            return sum(1 for n in a.GetNeighbors() if n.GetAtomicNum() != 1)
    
        # compute sum of distances to all heavy atoms
        sums = []
        for i in heavy_idxs:
            sums.append((float(dm[i, heavy_idxs].sum()), i))
    
        best_sum = min(s for s, _ in sums)
        cands = [i for s, i in sums if s == best_sum]
    
        # tie-break 1: prefer ring atoms
        ring_cands = [i for i in cands if m.GetAtomWithIdx(i).IsInRing()]
        if ring_cands:
            cands = ring_cands
    
        # tie-break 2/3: heavy degree, atomic number
        cands.sort(
            key=lambda idx: (
                -heavy_degree_idx(idx),
                -m.GetAtomWithIdx(idx).GetAtomicNum(),
                idx,  # deterministic
            )
        )
        return cands[0]
    ...
    def choose_root_atom(m: Chem.Mol) -> int:
        ...
        if ring_atoms:
            # connected components within ring atoms (via ring bonds)
            ring_adj = {i: [] for i in ring_atoms}
            for b in m.GetBonds():
                if not b.IsInRing():
                    continue
                i, j = b.GetBeginAtomIdx(), b.GetEndAtomIdx()
                if i in ring_adj and j in ring_adj:
                    ring_adj[i].append(j)
                    ring_adj[j].append(i)
            ...
            return max(
                largest,
                key=lambda idx: (heavy_degree(m.GetAtomWithIdx(idx)),
                                 m.GetAtomWithIdx(idx).GetAtomicNum()),
            )

        heavy = [a.GetIdx() for a in m.GetAtoms() if a.GetAtomicNum() != 1]
        return max(heavy, key=lambda idx: (heavy_degree(m.GetAtomWithIdx(idx)),
                                          m.GetAtomWithIdx(idx).GetAtomicNum())) if heavy else 0
    
    def bfs_parent_depth_heavy(m: Chem.Mol, root: int) -> Tuple[List[int], List[int]]:
        n = m.GetNumAtoms()
        parent = [-1] * n
        depth = [-1] * n

        # ensure root is heavy
        if m.GetAtomWithIdx(root).GetAtomicNum() == 1:
            for a in m.GetAtoms():
                if a.GetAtomicNum() != 1:
                    root = a.GetIdx()
                    break
        ...
        while q:
            u = q.popleft()
            for nb in m.GetAtomWithIdx(u).GetNeighbors():
                v = nb.GetIdx()
    ...
    def pick_A_upstream(m: Chem.Mol, B: int, C: int, parent: List[int], depth: List[int]) -> Optional[int]:
        b = m.GetAtomWithIdx(B)
        cands = [n.GetIdx() for n in b.GetNeighbors() if n.GetIdx() != C and n.GetAtomicNum() != 1]
        if not cands:
            return None
        p = parent[B]
        if p in cands and p != C:
            return p
        cands.sort(key=lambda i: (depth[i] if depth[i] != -1 else 10**9, -heavy_degree(m.GetAtomWithIdx(i))))
        return cands[0]

    def pick_D_downstream_heavy(m: Chem.Mol, B: int, C: int, depth: List[int]) -> Optional[int]:
        c = m.GetAtomWithIdx(C)
        cands = [n.GetIdx() for n in c.GetNeighbors() if n.GetIdx() != B and n.GetAtomicNum() != 1]
        if not cands:
            return None
        cands.sort(key=lambda i: -(depth[i] if depth[i] != -1 else -10**9))
        return cands[0]

    if root_atom is None:
        root_atom = choose_root_atom(mol)

    parent, depth = bfs_parent_depth_heavy(mol, root_atom)

    torsions: List[Torsion] = []

    # -----------------------
    # 1) Standard rotatable torsions (rooted)
    # -----------------------
    if include_rotatable_torsions:
        ...
        for i, j in mol.GetSubstructMatches(rot_pat):
            ...
            if depth[i] == -1 or depth[j] == -1:
                continue

            # orient away from root: B (upstream) -> C (downstream)
            if depth[i] < depth[j]:
                B, C = i, j
            else:
                B, C = j, i

            A = pick_A_upstream(mol, B, C, parent, depth)
            D = pick_D_downstream_heavy(mol, B, C, depth)
            if A is None or D is None:
                continue

            torsions.append((A, B, C, D))

        # distal-first ordering is often nicer for terminal tails
        torsions.sort(key=lambda t: depth[t[2]], reverse=True)

    # -----------------------
    # 2) Donor-H torsions (explicit H only, D is H)
    # -----------------------
    if include_donor_h_torsions:
        has_explicit_H = any(a.GetAtomicNum() == 1 for a in mol.GetAtoms())
        if has_explicit_H:
            rot_pat = Chem.MolFromSmarts(rot_smarts)
            movedHs: Set[int] = set()

            def is_donor_with_H(a: Chem.Atom) -> bool:
                if a.GetAtomicNum() not in (7, 8, 16):
                    return False
                return any(n.GetAtomicNum() == 1 for n in a.GetNeighbors())

            for i, j in mol.GetSubstructMatches(rot_pat):
                a1 = mol.GetAtomWithIdx(i)
                a2 = mol.GetAtomWithIdx(j)

                if is_donor_with_H(a1) and a2.GetAtomicNum() != 1:
                    donor, pivot = a1, a2
                elif is_donor_with_H(a2) and a1.GetAtomicNum() != 1:
                    donor, pivot = a2, a1
                else:
                    continue
                ...
                # A: heavy neighbor of pivot (prefer upstream), excluding donor
                A = pick_A_upstream(mol, B, C, parent, depth)
                if A is None:
                    continue

                # D: each explicit H on donor
                for hb in donor.GetNeighbors():
                    if hb.GetAtomicNum() != 1:
                        continue
                    D = hb.GetIdx()
                    ...
                    # optional strict check (your existing function)
                    if only_h_moves_on_rotation_fn is not None:
                        if not only_h_moves_on_rotation_fn(mol, tor):
                            continue
                    else:
                        # cheap structural safety: H should be terminal
                        if hb.GetDegree() != 1:
                            continue
